...
#os使用方法见：https://www.runoob.com/python/python-os-path.html
from urllib.request import urlopen
import csv
import os
...
def get_url(src, dest):
    ...
    if not os.path.isfile(dest):
        ...
        with open(dest, 'w') as f:
            f.write(urlopen(src).read().decode('utf-8'))
...
print(f"Average availability over 365 days is {np.mean(ds2['availability_365']):5.2f}")
print(f"Standard deviation on minimum nights is {np.std(ds2['minimum_nights']):5.2f}")
